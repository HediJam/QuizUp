...
public class DBHandler {
	...
	public static void main(final String[] args) throws Exception {
		...
		/*
		 * int id2 = getInstance().createQuiz("physics").getQzId(); Quiz qz2 =
		 * getInstance().findQuiz(id2); System.out.println(qz2.getCategory());
		 */
		...
		if (getInstance().existConfirmedUser("ÛŒÛŒÙˆØ²Ø±Ù†ÛŒÙ…", "Ù¾Ø³ÙˆØ±Ø¯"))
			System.out.println("YES");
		else
			System.out.println("NO");

		if (getInstance().confirmEmail("Ø§ÛŒÙ…ÛŒÙ„", "Ú©Ø¯"))
	...
	public void insertUser(User u) throws Exception {
		BasicDBObject doc = new BasicDBObject("_id", u.getUsername()).append("password", u.getPassword())
				.append("email", u.getEmail()).append("gender", u.getGender()).append("country", u.getCountry())
				.append("confirmationCode", u.getConfirmationCode()).append("mailConfirmed", u.isMailConfirmed());
	...
	public User findUser(String uid) {
		...
		return new User((String) user.get("_id"), (String) user.get("password"), (String) user.get("email"),
				(String) user.get("gender"), (String) user.get("country"), (String) user.get("confirmationCode"),
				((Boolean) user.get("mailConfirmed")).booleanValue());
	}

	public boolean confirmEmail(String email, String confirmationCode) {
		BasicDBObject query = new BasicDBObject("email", email).append("confirmationCode", confirmationCode);
	...
	boolean existConfirmedUser(String username, String password) {
		return existUser(new BasicDBObject("_id", username).append("password", password).append("mailConfirmed", true));
	}
	...
	public Quiz findQuiz(int qzID) {
		...
		return new Quiz(((Integer) qz.get("_id")).intValue(), ((Integer) qz.get("score1")).intValue(),
				((Integer) qz.get("score2")).intValue(), ((Integer) qz.get("finishTime1")).intValue(),
				((Integer) qz.get("finishTime2")).intValue(), (String) qz.get("category"), (String) qz.get("uid1"),
				(String) qz.get("uid2"), (ArrayList<Integer>) qz.get("qsIDs"));
	}
	...
	public void updateQuiz(int qzID, Quiz q) {
		...
		BasicDBObject newQuiz = new BasicDBObject("$set",
				new BasicDBObject("score1", q.getScore1()).append("score2", q.getScore2())
						.append("finishTime1", q.getFinishTime1()).append("finishTime2", q.getFinishTime2())
						.append("category", q.getCategory()).append("uid1", q.getUid1()).append("uid2", q.getUid2())
	...
	/*
	 * public void updateQuiz(Quiz q, String username, int score, int
	 * finishTime) {
	 * 
	 * BasicDBObject query = new BasicDBObject("_id", q.getQzId()); DBCursor
	 * curs = quizs.find(query);
	 * 
	 * try { if (curs.hasNext()) { DBObject qz = curs.next();
	 * if(qz.get("uid1").equals("")) { quizs.update((DBObject) qz,
	 * (BasicDBObject) new BasicDBObject("$set", new BasicDBObject("uid1",
	 * username) .append("score1", score) .append("finishTime1", finishTime) ));
	 * 
	 * } else if(qz.get("uid2").equals("")) { quizs.update((DBObject) qz,
	 * (BasicDBObject) new BasicDBObject("$set", new BasicDBObject("uid2",
	 * username) .append("score2", score) .append("finishTime2", finishTime) ));
	 * } else System.out.println("This quiz has done by two user before");
	 * 
	 * }
	 * 
	 * } finally { curs.close(); } }
	...
	private void insertQuiz(Quiz q) // throws Exception
	{
		...
		BasicDBObject doc = new BasicDBObject("_id", q.getQzId()).append("score1", q.getScore1())
				.append("score2", q.getScore2()).append("finishTime1", q.getFinishTime1())
				.append("finishTime2", q.getFinishTime2()).append("category", q.getCategory())
				.append("uid1", q.getUid1()).append("uid2", q.getUid2()).append("qsIDs", q.getQsIDs());
	...
	private void insertQuestion(Question qs) {
		...
		BasicDBObject doc = new BasicDBObject("_id", qs.getQsID()).append("text", qs.getText())
				.append("category", qs.getCategory()).append("answer", qs.getAnswer())
	...
	public Question findQuestion(int qsID) {
		...
		return new Question(((Integer) qs.get("_id")).intValue(), (String) qs.get("text"), (String) qs.get("category"),
				(String) qs.get("answer"), (ArrayList<String>) qs.get("choices"));
	}
	...
	public void updateQuestion(int qsID, Question qs) {
		...
		BasicDBObject newQuestion = new BasicDBObject("$set",
				new BasicDBObject("text", qs.getText()).append("category", qs.getCategory())
						.append("answer", qs.getAnswer()).append("choices", qs.getChoices()));
		;
		try {
			if (curs.hasNext()) {
		...
		try {
			while (curs.hasNext()) {
				...
				questionsArr.add(new Question(((Integer) qs.get("_id")).intValue(), (String) qs.get("text"),
						(String) qs.get("category"), (String) qs.get("answer"), (ArrayList<String>) qs.get("choices")));
			}
	...
	public void updateCategory(String category, String newCategory) {
		...
		try {
			while (curs.hasNext()) {
				DBObject q = curs.next();
				questions.update((DBObject) q,
						(BasicDBObject) new BasicDBObject("$set", new BasicDBObject("category", newCategory)));
			}
		} finally {
			curs.close();
		}
	...
	private void insertRequest(int qzID/* , String uid1 */) {
		...
		BasicDBObject doc = new BasicDBObject("_id", qzID).append("accept", false);
		try {
			requests.insert(doc);
		} catch (MongoException ex) {
			System.out.println("duplicattte quiz id");
	...
	public void acceptRequest(int qzID, String uid2) {
		DBCursor curs = requests.find(new BasicDBObject("_id", qzID).append("accept", false));
		try {
			if (curs.hasNext()) {
				// requests.remove(curs.next());
				DBObject rq = curs.next();
				requests.update((DBObject) rq,
						(BasicDBObject) new BasicDBObject("$set", new BasicDBObject("accept", true)));
	...
	public int numOfWin(String uid) {
		int res = 0;
		DBCursor curs = quizs
				.find(new BasicDBObject("$or", asList(new BasicDBObject("uid1", uid), new BasicDBObject("uid2", uid))));
		try {
			while (curs.hasNext()) {
				DBObject qz = curs.next();
				Quiz quiz = new Quiz(((Integer) qz.get("_id")).intValue(), ((Integer) qz.get("score1")).intValue(),
						((Integer) qz.get("score2")).intValue(), ((Integer) qz.get("finishTime1")).intValue(),
						((Integer) qz.get("finishTime2")).intValue(), (String) qz.get("category"),
						(String) qz.get("uid1"), (String) qz.get("uid2"), (ArrayList<Integer>) qz.get("qsIDs"));
				// System.out.println("id "+quiz.getQzId());
				// System.out.println("win "+quiz.winner());
				if (quiz.winner().equals(uid))
					res++;
			}
	...
	public int numOfStronWin(String uid) {
		...
		try {
			while (curs.hasNext()) {
				...
				// System.out.println("id "+quiz.getQzId());
				// System.out.println("win "+quiz.winner());
				if (quiz.strongWinner().equals(uid))
					res++;
			}
		...
		try {
			while (cursor.hasNext()) {
				...
				if (!currUid.equals(uid)) {
					if (numOfWin(currUid) > numOfwin)
						rank++;
					else if ((numOfWin(currUid) == numOfwin) && (sumScore(currUid) > sumScore))
						rank++;
				}
			}
		} finally {
